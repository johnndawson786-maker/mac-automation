...
if totalRecipients is 0 then
	display dialog "The group \"" & groupName & ¬
		"\" has no contacts with usable phone numbers." buttons {"OK"} default button "OK" with icon stop
	return
end if
...
on resolveGroupName(fileName, dateStr)
	...
	try
		set answer to text returned of (display dialog ¬
			...
			¬
				"Enter the Contacts group name to message:" default answer defaultGuess buttons {"Cancel", "Use This"} default button "Use This" with icon caution)
